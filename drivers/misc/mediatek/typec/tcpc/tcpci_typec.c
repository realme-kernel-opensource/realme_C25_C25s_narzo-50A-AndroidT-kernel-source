	...
	struct tcpc_device *tcpc, uint8_t typec_role, bool postpone)
{
	...
	TYPEC_INFO("typec_new_role: %s typec_attach_old = %d\r\n"
				,typec_role_name[typec_role], tcpc->typec_attach_old);

	if (tcpc->typec_attach_old == TYPEC_UNATTACHED || tcpc->typec_attach_old == TYPEC_ATTACHED_SNK
